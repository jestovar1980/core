...
class Share20OcsControllerTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$this->sharingWhitelist = $this->createMock(SharingWhitelist::class);
	...
	*/
	...
	public function testCreateShareLinkNotInWhitelist() {
		$this->request
			->method('getParam')
			->will($this->returnValueMap([
				['path', null, 'valid-path'],
		...
		$this->sharingWhitelist->method('isGroupPublicShareSharersWhitelistEnabled')->willReturn(true);
		$this->sharingWhitelist->method('isUserInWhitelistedPublicShareSharersGroups')->willReturn(false);

		$expected = new Result(null, 404, 'User is not in a whitelisted group');
		$result = $this->ocs->createShare();
